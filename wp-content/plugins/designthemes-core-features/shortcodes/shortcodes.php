...
class DTCoreShortcodesDefination {
	...
	function dt_sc_pricing_table_item($attrs, $content = null) {
		...
		$out = "<div class='dt-sc-pr-tb-col type1 $selected'>";
	...
	function dt_sc_woo_cart($attrs, $content = null) {
		...
		if( function_exists("is_woocommerce")) :
		    $out = '<a class="cart-info" href="'.WC()->cart->get_cart_url().'" title="'.esc_attr__( 'View Shopping Cart', 'designthemes-core' ).'">';
	...
	function dt_sc_post( $attrs, $content = null ){
		...
		if( $style == 'blog-default-style') :
			$thumbnail = 'begin-blog-default';
		else:
			$thumbnail = 'full';
		endif;
				...
				$out .= '<article id="post-'.esc_attr($id).'" class="'.esc_attr($post_classes).'">';
					...
					if( $format == "image" || empty($format) ) :
						if( has_post_thumbnail() ) :
							...
							$out .= '	<a href="'.esc_url($link).'">'.get_the_post_thumbnail( $id, $thumbnail ).'</a>';
						...
						if( $post_meta['post-gallery-items'] != '' ) :
							...
							$out .= '	<ul class="entry-gallery-post-slider">';
											...
											foreach ( $items as $item ) {
												$out .= '<li>'.wp_get_attachment_image( $item, $thumbnail ).'</li>';
											}
							$out .= '	</ul>';
							$out .= '	<div class="entry-format '.esc_attr($hide_post_format).'">';
							$out .= '		<a class="ico-format" href="'.esc_url(get_post_format_link( $format )).'"></a>';
						...
						if( $post_meta['media-url'] != '' ) :
							...
							$out .= '	<div class="dt-video-wrap">';
											if( $post_meta['media-type'] == 'oembed' ) :
												$out .= wp_oembed_get($post_meta['media-url']);
											elseif( $post_meta['media-type'] == 'self' ) :
												$out .= wp_video_shortcode( array('src' => $post_meta['media-url']) );
											endif;
							$out .='	</div>';
							$out .= '	<div class="entry-format '.esc_attr($hide_post_format).'">';
							$out .= '		<a class="ico-format" href="'.esc_url(get_post_format_link( $format )).'"></a>';
						...
						if( $post_meta['media-url'] != '' ) :
							$out .= '<div class="entry-thumb">';
								if( $post_meta['media-type'] == 'oembed' ) :
									$out .= wp_oembed_get($post_meta['media-url']);
								elseif( $post_meta['media-type'] == 'self' ) :
									$out .= wp_audio_shortcode( array('src' => $post_meta['media-url']) );
								endif;
							$out .= '	<div class="entry-format '.esc_attr($hide_post_format).'">';
							$out .= '		<a class="ico-format" href="'.esc_url(get_post_format_link( $format )).'"></a>';
							$out .= '	</div>';
					...
					if( $style == "entry-date-left") :
						...
						$out .= '	</div>';
									...
									if( strtolower($allow_excerpt) == 'yes' ):
										$out .= '<div class="entry-body">'.begin_excerpt($excerpt_length).'</div>';
									endif;
					...
					elseif( $style == "entry-date-author-left"):
						...
						$out .= 			esc_html__('Posted on','designthemes-core').get_the_date(' d M Y');
						$out .= '		</div>';
						$out .= '		<div class="comments '.esc_attr($hide_comment_meta).'">';
											$commenttext = "";
											if((wp_count_comments($id)->approved) == 0):
											...
											$out .= '/ <a class="comments" href="'.esc_url($link).'/#respond">';
											$out .= '	<i class="pe-icon pe-chat"></i> '.esc_html($commenttext).' '.esc_html__('Comments','designthemes-core');
											$out .= '</a>';
						$out .= '		</div>';
		...
		if( !empty( $id ) ){
			...
			if( $p->post_type === "dt_portfolios" ) {
				...
				if( has_post_thumbnail( $id ) ){
				  $post_thumb = wp_get_attachment_image_src(get_post_thumbnail_id( $id ), 'full', false);
							...
							$out .= '<div class="dt-sc-event type1">';
								 ...
								 $out .= '<div class="dt-sc-event-thumb">';
									if(has_post_thumbnail()):
										...
										$out .= get_the_post_thumbnail($post->ID, 'designthemes-event-list2', $attr);
									else:
	...
	function dt_sc_down_count($attrs, $content = null) {
		...
		$out .= '<div class="downcount '.$class.'" data-date="'.begin_cs_get_option('comingsoon-launchdate', date('m/d/Y')).'" data-offset="'.begin_cs_get_option('comingsoon-timezone', '0').'">';
