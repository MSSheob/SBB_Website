...
class BEGIN_Kirki {
	...
	public function __construct() {
		...
		// Add our CSS
		add_action( 'wp_enqueue_scripts', array( $this, 'enqueue_styles' ), 102 ); // old priority = 20
		// Add google fonts
	...
	public function enqueue_styles() {
		...
		if ( ! empty( $styles ) ) {
			...
			wp_add_inline_style( 'begin-custom', $styles );
		}
	}
	 ...
	 */
	public static function kirki_admin_enqueue_styles() {
		// If Kirki exists there's no need to proceed any further
//		if ( class_exists( 'Kirki' ) ) {
//			return;
//		}
			...
			if ( isset( $field['type'] ) && 'typography' == $field['type'] ) {
				...
				if ( isset( $value['font-family'] ) ) {
					...
					if ( ! empty( $value['variant'] ) ) {
						$all_variants = array( '100', '100italic', '200', '200italic', '300', '300italic', 'regular', 'italic', '500', '500italic', '600', '600italic', '700', '700italic', '800', '800italic', '900', '900italic' );
						$weights = join( ',', $all_variants );
						$url .= ':' . $weights;
					}
	...
	public static function admin_enqueue_fonts() {
		// Check if we need to exit early
		if ( empty( self::$fields ) || ! is_array( self::$fields ) ) {
			return;
		}
	...
	public static function admin_editor_fonts_url() {
		// Check if we need to exit early
		$font_urls = array();
		...
		foreach ( self::$fields as $field ) {
			...
			if ( isset( $field['type'] ) && 'typography' == $field['type'] ) {
				...
				if ( isset( $value['font-family'] ) ) {
					$url = '//fonts.googleapis.com/css?family=' . str_replace( ',', '', trim($value['font-family']) );
