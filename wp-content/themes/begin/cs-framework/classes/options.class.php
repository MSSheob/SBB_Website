...
abstract class CSFramework_Options extends CSFramework_Abstract {
  ...
  public function element_class( $el_class = '' ) {
    ...
    return ( $field_class || $el_class ) ? ' class="'. esc_attr( $el_class . $field_class ) .'"' : '';
  ...
  public function element_after() {
    ...
    $out  = ( isset( $this->field['info'] ) ) ? '<p class="cs-text-desc">'. esc_html( $this->field['info'] ).'</p>' : '';
  ...
  public function element_debug() {
    ...
    if( ( isset( $this->field['debug'] ) && $this->field['debug'] === true ) || ( defined( 'CS_OPTIONS_DEBUG' ) && CS_OPTIONS_DEBUG ) ) {
      ...
      $out .= "<pre>";
      $out .= "<strong>". esc_html__( 'CONFIG', 'begin' ) .":</strong>";
      $out .= "\n";
      ob_start();
      var_export( $this->field );
      $out .= htmlspecialchars( ob_get_clean() );
      $out .= "\n\n";
      $out .= "<strong>". esc_html__( 'USAGE', 'begin' ) .":</strong>";
      $out .= "\n";
      $out .= ( isset( $this->field['id'] ) ) ? "cs_get_option( '". $this->field['id'] ."' );" : '';

      if( ! empty( $value ) ) {
        $out .= "\n\n";
        $out .= "<strong>". esc_html__( 'VALUE', 'begin' ) .":</strong>";
        $out .= "\n";
        ob_start();
        var_export( $value );
        $out .= htmlspecialchars( ob_get_clean() );
    ...
    if( ( isset( $this->field['debug_light'] ) && $this->field['debug_light'] === true ) || ( defined( 'CS_OPTIONS_DEBUG_LIGHT' ) && CS_OPTIONS_DEBUG_LIGHT ) ) {
      ...
      $out .= ( isset( $this->field['id'] ) ) ? "cs_get_option( '". $this->field['id'] ."' );" : '';
      $out .= "\n";
      $out .= "<strong>". esc_html__( 'ID', 'begin' ) .":</strong>";
      $out .= "\n";
  ...
  public function element_help() {
    return ( isset( $this->field['help'] ) ) ? '<span class="cs-help" data-title="'. esc_attr( $this->field['help'] ).'"><span class="fa fa-question-circle"></span></span>' : '';
  }
  ...
  public function element_after_multilang() {
    ...
    if ( is_array( $this->multilang ) ) {
      ...
      $out .= '<input type="hidden" name="'. esc_attr( $this->element_name( '[multilang]', true ) ).'" value="true" />';
      $out .= '</fieldset>';
      $out .= '<p class="cs-text-desc">'. sprintf( esc_html__( 'You are editing language: ( %s )', 'begin' ), '<strong>'. $this->multilang['current'] .'</strong>' ) .'</p>';
