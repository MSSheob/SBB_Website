...
class CSFramework_Metabox extends CSFramework_Abstract{
  ...
  public function __construct( $options ){
    ...
    if( ! empty( $this->options ) ) {
      $this->addAction( 'add_'.'meta_'.'boxes', 'add_cs_meta_box' );
  ...
  // add metabox
  public function add_cs_meta_box( $post_type ) {

    foreach ( $this->options as $value ) {
      call_user_func( 'add_'.'meta_'.'box', $value['id'], $value['title'], array( &$this, 'render_meta_box_content' ), $value['post_type'], $value['context'], $value['priority'], $value );
    }
  ...
  public function render_meta_box_content( $post, $callback ) {
    ...
    echo '<div class="cs-framework cs-metabox-framework">';
      ...
      echo '<input type="hidden" name="cs_section_id['. esc_attr( $unique ) .']" class="cs-reset" value="'. esc_attr( $section_id ) .'">';

      echo '<div class="cs-body'. esc_attr( $show_all ) .'">';
          ...
          echo '<div class="cs-nav">';
            ...
            foreach( $sections as $value ) {
              ...
              if( isset( $value['fields'] ) ) {
                $active_section = ( ( empty( $section_id ) && $num === 0 ) || $section_id == $value['name'] ) ? 'cs-section-active' : '';
                echo '<li><a class="'.esc_attr( $active_section ).'" href="#" data-section="'. esc_attr( $value['name'] ).'">'. wp_kses( $tab_icon, array( 'i' => array( 'id'  => array(), 'class' => array() ) ) ) . esc_html( $value['title'] ) .'</a></li>';
              } else {
                echo '<li><div class="cs-seperator">'. wp_kses( $tab_icon, array( 'i' => array( 'id'  => array(), 'class' => array() ) ) ) . esc_html( $value['title'] ) .'</div></li>';
              }
        ...
        echo '<div class="cs-content">';
          ...
          foreach( $sections as $v ) {
            ...
            if( isset( $v['fields'] ) ) {
              ...
              $active_content = ( ( empty( $section_id ) && $num === 0 ) || $section_id == $v['name'] ) ? 'display: block;' : '';

              echo '<div id="cs-tab-'. esc_attr( $v['name'] ).'" class="cs-section" style="'.esc_attr( $active_content ).'">';
              echo ( isset( $v['title'] ) ) ? '<div class="cs-section-title"><h3> '. esc_html( $v['title'] ).'</h3></div>' : '';
