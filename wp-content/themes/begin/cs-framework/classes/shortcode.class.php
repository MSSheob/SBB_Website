...
class CSFramework_Shortcode_Manager extends CSFramework_Abstract{
  ...
  public function media_shortcode_button( $editor_id ) {
    ...
    if( ! in_array( $post_type, $this->exclude_post_types ) ) {
      echo '<a href="#" class="button button-primary cs-shortcode" data-editor-id="'. esc_attr( $editor_id ).'">'. esc_html__( 'Add Shortcode', 'begin' ) .'</a>';
    }
  ...
  ?>
    <div id="cs-shortcode-dialog" class="cs-dialog" title="<?php esc_attr_e( 'Add Shortcode', 'begin' ); ?>">
      <div class="cs-dialog-header">
        <?php $_s_class = ( is_rtl() ) ? 'cs-dialog-select chosen-rtl ' : 'cs-dialog-select'; ?>
        <select class="<?php echo esc_attr( $_s_class ); ?>" data-placeholder="<?php esc_attr_e( 'Select a shortcode', 'begin' ); ?>">
          <option value=""></option>
          <?php
            foreach ( $this->options as $group ) {
              echo '<optgroup label="'. esc_html( $group['title'] ).'">';
      ...
      <div class="cs-insert-button hidden">
        <a href="#" class="button button-primary cs-dialog-insert"><?php esc_html_e( 'Insert Shortcode', 'begin' ); ?></a>
      </div>
    </div>
  <?php
  }
  ...
  public function shortcode_generator() {
    ...
    if( isset( $shortcode['clone_fields'] ) ) {
      ...
      echo '<div class="cs-shortcode-clone" data-clone-id="'. esc_attr( $clone_id ) .'">';
      ...
      echo '<div class="cs-clone-button"><a id="shortcode-clone-button" class="button" href="#"><i class="fa fa-plus-circle"></i> '. esc_html( $shortcode['clone_title'] ).'</a></div>';
