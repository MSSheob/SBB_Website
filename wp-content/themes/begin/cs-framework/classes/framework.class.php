...
class CSFramework extends CSFramework_Abstract {
  ...
  public function validate_save( $request ) {
    ...
    if ( isset( $request['import'] ) && ! empty( $request['import'] ) ) {
      ...
      $add_errors[] = $this->add_settings_error( esc_html__( 'Success. Imported backup options.', 'begin' ), 'updated' );
    }

    // reset all options
    if ( isset( $request['resetall'] ) ) {
      $add_errors[] = $this->add_settings_error( esc_html__( 'Default options restored.', 'begin' ), 'updated' );
      return;
    }
    ...
    if ( isset( $request['reset'] ) && ! empty( $section_id ) ) {
      ...
      $add_errors[] = $this->add_settings_error( esc_html__( 'Default options restored for only this section.', 'begin' ), 'updated' );
    }
  ...
  public function admin_page() {
    ...
    echo '<div class="cs-framework cs-option-framework">';
        ...
        if ( ! empty( $cs_errors ) ) {
          foreach ( $cs_errors as $error ) {
            if( in_array( $error['setting'], array( 'general', 'cs-errors' ) ) ) {
              echo '<div class="cs-settings-error '. esc_attr( $error['type'] ) .'">';
      ...
      if( $this->settings['ajax_save'] ) {
        ...
        echo '<span id="cs-save-ajax">'. esc_html__( 'Settings saved.', 'begin' ) .'</span>';
      }

      submit_button( esc_html__( 'Save', 'begin' ), 'primary cs-save', 'save', false, array( 'data-save' => esc_html__( 'Saving...', 'begin' ) ) );
      submit_button( esc_html__( 'Restore', 'begin' ), 'secondary cs-restore cs-reset-confirm', $this->unique .'[reset]', false );

      if( $this->settings['show_reset_all'] ) {
        submit_button( esc_html__( 'Reset All Options', 'begin' ), 'secondary cs-restore cs-warning-primary cs-reset-confirm', $this->unique .'[resetall]', false );
      }

      echo '</fieldset>';
      echo ( empty( $has_nav ) ) ? '<a href="#" class="cs-expand-all"><i class="fa fa-eye-slash"></i> '. esc_html__( 'show all options', 'begin' ) .'</a>' : '';
      echo '<div class="clear"></div>';
      echo '</header>'; // end .cs-header

      echo '<div class="cs-body'. esc_attr( $has_nav ) .'">';
        ...
        echo '<div class="cs-nav">';
          ...
          foreach ( $this->options as $key => $tab ) {
            ...
            if( ( isset( $tab['sections'] ) ) ) {
              ...
              $active_style = ( ! empty( $tab_active ) ) ? 'display: block;' : '';
              $active_list  = ( ! empty( $tab_active ) ) ? ' cs-tab-active' : '';
              $tab_icon     = ( ! empty( $tab['icon'] ) ) ? '<i class="cs-icon '. $tab['icon'] .'"></i>' : '';

              echo '<li class="cs-sub'. esc_attr( $active_list ) .'">';

                echo '<a href="#" class="cs-arrow">'. wp_kses( $tab_icon ,array( 'i' => array( 'id'  => array(), 'class' => array() ) ) ) . esc_html( $tab['title'] ) .'</a>';

                echo '<ul style="'.esc_attr( $active_style ).'">';
                foreach ( $tab['sections'] as $tab_section ) {

                  $active_tab = ( $section_id == $tab_section['name'] ) ? 'cs-section-active' : '';
                  $icon = ( ! empty( $tab_section['icon'] ) ) ? '<i class="cs-icon '. $tab_section['icon'] .'"></i>' : '';

                  echo '<li><a href="#" class="'. esc_attr( $active_tab ) .'" data-section="'. esc_attr( $tab_section['name'] ) .'">'. wp_kses( $icon, array( 'i' => array( 'id'  => array(), 'class' => array() ) ) ) . esc_html( $tab_section['title'] ) .'</a></li>';
              ...
              if( isset( $tab['fields'] ) ) {
                ...
                $active_list = ( $section_id == $tab['name'] ) ? 'cs-section-active' : '';
                echo '<li><a href="#" class="'. esc_attr( $active_list ) .'" data-section="'. esc_attr( $tab['name'] ) .'">'. wp_kses( $icon, array( 'i' => array( 'id'  => array(), 'class' => array() ) ) ) . esc_html( $tab['title'] ).'</a></li>';

              } else {

                echo '<li><div class="cs-seperator">'. wp_kses( $icon, array( 'i' => array( 'id'  => array(), 'class' => array() ) ) ) . esc_html( $tab['title'] ) .'</div></li>';
              }
        ...
        echo '<div class="cs-content">';
          ...
          foreach( $this->sections as $section ) {
            ...
            if( isset( $section['fields'] ) ) {
              ...
              $active_content = ( $section_id == $section['name'] ) ? 'display: block;' : '';
              echo '<div id="cs-tab-'. esc_attr( $section['name'] ) .'" class="cs-section" style="'.esc_attr( $active_content ).'">';
              echo ( isset( $section['title'] ) && empty( $has_nav ) ) ? '<div class="cs-section-title"><h3>'. esc_html( $section['title'] ).'</h3></div>' : '';
      ...
      echo '<div class="cs-block-left"></div>';
      echo '<div class="cs-block-right">'.esc_html__( 'Version', 'begin' ).' '. CS_VERSION .'</div>';
      echo '<div class="clear"></div>';
