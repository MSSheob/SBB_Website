...
class CSFramework_Option_typography extends CSFramework_Options {
  ...
  public function output() {
    ...
    if( is_object( $google_json ) ) {
      ...
      echo '<select name="'. $this->element_name( '[family]' ) .'" class="'. esc_attr( $is_chosen . $chosen_rtl ).'cs-typo-family" data-atts="family"'. $this->element_attributes() .'>';

      do_action( 'cs_typography_family', $family_value, $this );

      echo '<optgroup label="'. esc_html__( 'Web Safe Fonts', 'begin' ) .'">';
      ...
      echo '<optgroup label="'. esc_html__( 'Google Fonts', 'begin' ) .'">';
      ...
      if( ! empty( $is_variant ) ) {
        ...
        foreach ( $variants as $variant ) {
          echo '<option value="'. esc_attr( $variant ).'"'. $this->checked( $variant_value, $variant, 'selected' ) .'>'. esc_html( $variant ).'</option>';
        }
        echo '</select>';
        echo '</label>';
      ...
      echo esc_html__( 'Error! Can not load json file.', 'begin' );
