      ...
      foreach ( $jsons as $path ) {
        ...
        if( is_object( $object ) ) {
          ...
          foreach ( $object->icons as $icon ) {
            echo '<a class="cs-icon-tooltip" data-cs-icon="'. esc_attr( $icon ) .'" data-title="'. esc_attr( $icon ) .'"><span class="cs-icon cs-selector"><i class="'. esc_attr( $icons) .'"></i></span></a>';
          }

        } else {
          echo '<h4 class="cs-icon-title">'. esc_html__( 'Error! Can not load json file.', 'begin' ) .'</h4>';
        }
...
if( ! function_exists( 'cs_set_icons' ) ) {
  function cs_set_icons() {
    ...
    echo '<div id="cs-icon-dialog" class="cs-dialog" title="'. esc_attr__( 'Add Icon', 'begin' ) .'">';
    echo '<div class="cs-dialog-header cs-text-center"><input type="text" placeholder="'. esc_attr__( 'Search a Icon...', 'begin' ) .'" class="cs-icon-search" /></div>';
    echo '<div class="cs-dialog-load"><div class="cs-icon-loading">'. esc_html__( 'Loading...', 'begin' ) .'</div></div>';
    echo '</div>';
