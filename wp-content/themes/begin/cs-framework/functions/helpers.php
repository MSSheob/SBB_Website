...
if ( ! function_exists( 'cs_add_element' ) ) {
  function cs_add_element( $field = array(), $value = '', $unique = '' ) {
    ...
    if( isset( $field['title'] ) ) {
      $field_desc = ( isset( $field['desc'] ) ) ? '<p class="cs-text-desc">'. esc_html( $field['desc'] ) .'</p>' : '';
      $output .= '<div class="cs-title"><h4>' . esc_html( $field['title'] ) . '</h4>'. $field_desc .'</div>';
    }
    ...
    if( class_exists( $class ) ) {
      ...
      $output .= '<p>'. esc_html__( 'This field class is not available!', 'begin' ) .'</p>';
    }
