...
jQuery(document).ready(function($){
	...
	$(".dt-select-service").change(function(){
		...
		$.ajax({
			...
			type: 'html',
			data:{ action: 'begin_fill_staffs', service_id :$id },
	...
	if($('body').hasClass('page-template-tpl-reservation-type2-php') ){
		...
		function begin_calculate_step_value() {
		...
		$("body").delegate(".generate-schedule","click",function(e){
			...
			$.ajax({
				...
				url: dttheme_urls.ajaxurl,
				type: 'html',
				data:{ action: 'begin_dt_generate_schedule', serviceid :$serviceid, staffid :$staffid, staffids :$staffids, datepicker :$aptdatepicker },
				beforeSend: function(){
		 ...
		 $(".dt-sc-about-project-form .schedule-it").click(function(e){
			...
			if( typeof($start) != 'undefined' ) {
				$.ajax({
					...
					url: 	dttheme_urls.ajaxurl,
					data: 	{ 
						action: "begin_dttheme_new_reservation",
						staffid: $staffid,
						serviceid: $serviceid,
						start: $start,
						end: $end,
	...
	function begin_staff_reservation(){
		$date = $("*[name=date]").val();
		$stime = $('*[name=start-time]').val();
		$etime = $('*[name=end-time]').val();
		$staff = $('*[name=staff]').val();
		...
		if( $staff.length > 0 || $service.length > 0 ) {
				$.ajax({
					...
					url: dttheme_urls.ajaxurl,
					type: 'html',
					data:{ action: 'begin_available_times', 
						date :$date,
						stime:$stime,
						etime:$etime,
						staffid:$staff,
